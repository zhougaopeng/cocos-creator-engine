...
const CONST_OBJ_REG =
    /^[ \t]*\/\*@__DROP_PURE_EXPORT__\*\/[\w\s]*?\s+([A-Za-z_$][0-9A-Za-z_$]*)\s*=\s*({[\n\s\S]*?})/gm;
const CONST_NUM_REG =
    /^[ \t]*const[ \t]+([A-Z_$][0-9A-Z_$]*)[ \t]*=[ \t]*(\d{1,2});?$/gm;
...
exports.inlineEnum = function (filename) {
    if (filename.endsWith(".js") || filename.endsWith(".ts")) {
        return es.map(function (data, callback) {
            ...
            if (content.includes(COMMENT_ANNOTATION)) {
                // fast test
                let error = null;
                matchAll(content, CONST_OBJ_REG, function (match, obj, body) {
                    try {
                        ...
                        error = new Error(
                            `Failed to inline property "${obj}" in "${filename}", ${e}`
                        );
                        return;
                    }
                    for (const key in body) {
                        ...
                        if (replaced !== content) {
                            content = replaced;
                            console.log(
                                `[inline-prop] '${obj}.${key}' is inlined with ${body[key]}`
                            );
                        }
                    }
                });
            ...
            if (content.includes(COMMENT_ANNOTATION)) {
                ...
                matchAll(content, CONST_NUM_REG, function (match, name, value) {
                    let reg = new RegExp(
                        `([{,]\\s*|\\btypeof[ \\t]+)?(\\b${name}\\b)(?![ \\t]*=)`,
                        "g"
                    ); // not support negative lookbehind...
                    ...
                    if (replaced !== content) {
                        content = replaced;
                        console.log(
                            `[inline-prop] '${name}' is inlined with ${value}`
                        );
                    }
                });
                if (error) {
