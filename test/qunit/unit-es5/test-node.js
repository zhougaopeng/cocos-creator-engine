...
test("test walk", function () {
    ...
    strictEqual(
        walkHistory,
        "ABDD*EGG*HH*E*FII*F*B*CC*A*",
        "walk history should be the same as predicted"
    );
});
...
test("activeInHierarchy", function () {
    ...
    strictEqual(
        ...
        false,
        "parent should not activeInHierarchy before add to scene"
    );
    strictEqual(
        child.activeInHierarchy,
        false,
        "child should not activeInHierarchy before add to scene"
    );
    ...
    strictEqual(
        parent.activeInHierarchy,
        true,
        "parent should become activeInHierarchy after add to scene"
    );
    strictEqual(
        child.activeInHierarchy,
        true,
        "child should become activeInHierarchy after add to scene"
    );
    ...
    strictEqual(
        child.activeInHierarchy,
        false,
        "child still deactiveInHierarchy"
    );
    ...
    strictEqual(
        child.activeInHierarchy,
        false,
        "child deactiveInHierarchy because parent deactived"
    );
    ...
    strictEqual(
        parent.activeInHierarchy,
        true,
        "parent become activeInHierarchy"
    );
    strictEqual(
        child.activeInHierarchy,
        true,
        "child become activeInHierarchy because parent actived"
    );
    ...
    strictEqual(
        parent.activeInHierarchy,
        false,
        "parent should not activeInHierarchy after remove from scene"
    );
    strictEqual(
        child.activeInHierarchy,
        false,
        "child should not activeInHierarchy after remove from scene"
    );
});
...
test("activeInHierarchy should be false after destroy", function () {
    ...
    strictEqual(
        parent.activeInHierarchy,
        false,
        "parent should not activeInHierarchy after destroy"
    );
    strictEqual(
        child.activeInHierarchy,
        false,
        "child should not activeInHierarchy after destroy"
    );
});
...
test("nodes should be destroyed with the scene", function () {
    ...
    strictEqual(
        child.activeInHierarchy,
        false,
        "nodes should not activeInHierarchy after destroy"
    );
    strictEqual(
        child.isValid,
        false,
        "nodes should be destroyed with the scene"
    );
});
...
test("activation logic for component", function () {
    ...
    var MyComponent = cc.Class({
        ...
        ctor: function () {
            this.expect(
                CallbackTester.OnLoad,
                "call onLoad while attaching to node"
            );
            this.expect(
                CallbackTester.OnEnable,
                "then call onEnable if node active",
                true
            );
        },
        start: function () {
    ...
    ok(
        obj.getComponent(MyComponent) === comp,
        "getComponent: can get my component"
    );
    ok(
        obj.getComponent(cc.js.getClassName(MyComponent)) === comp,
        "getComponent: can get my component by name"
    );
    ok(
        obj.getComponent(MyComponentBase) === comp,
        "getComponent: can get component by base type"
    );
    ok(
        obj.getComponent(cc.js.getClassName(MyComponentBase)) === comp,
        "getComponent: can get component by base name"
    );

    comp.expect(
        CallbackTester.OnDisable,
        "should called onDisable when destroy"
    );
    comp.destroy(); // onDisable

    strictEqual(
        obj.getComponent(MyComponent),
        comp,
        "can still get component in this frame"
    );
    ...
    strictEqual(
        obj.getComponent(MyComponent),
        null,
        "can not get component after this frame"
    );
...
test("get self components", function () {
    ...
    ok(
        obj.getComponents(MyComponent).length === 3,
        "getComponents: can get my component array"
    );
    ok(
        obj.getComponentInChildren(MyComponent) === comp,
        "getComponentInChildren: can get my component in children"
    );
    ok(
        obj.getComponentsInChildren(MyComponent).length === 5,
        "getComponentsInChildren: can get my components in children and self"
    );
...
test("should not include self component", function () {
    ...
    ok(
        obj.getComponentInChildren(MyComponent) === null,
        "getComponentInChildren should not include self component"
    );
...
test("should include self component", function () {
    ...
    ok(
        obj.getComponentsInChildren(MyComponent).length === 3,
        "getComponentsInChildren should include self component"
    );
...
test("destroy", function () {
    ...
    childComp.expect(
        CallbackTester.OnDisable,
        "should disable while destroy parent"
    );
    childComp.notExpect(
        CallbackTester.OnDestroy,
        "can not destroyed before the end of this frame"
    );
    ...
    childComp.notExpect(
        CallbackTester.OnDisable,
        "child comp should only disabled once"
    );
    childComp.notExpect(
        CallbackTester.OnEnable,
        "child component should not re-enable when parent destroying"
    );
    childComp.expect(
        CallbackTester.OnDestroy,
        "should destroyed at the end of frame"
    );
    ...
    var ChildComp_new = cc.Class({
        ...
        ctor: function () {
            this.expect(
                CallbackTester.OnLoad,
                "new child component should onLoad even if added after destroy"
            );
            this.expect(
                CallbackTester.OnEnable,
                "new child component should enable even if added after destroy",
                true
            );
            this.expect(
                CallbackTester.OnDisable,
                "new component's onDisable should be called before its onDestroy",
                true
            );
            this.expect(
                CallbackTester.OnDestroy,
                "new component should also destroyed at the end of frame",
                true
            );
        },
    });
    ...
    FO._deferredDestroy(true);

    strictEqual(
        childComp_new.isValid,
        false,
        "node will finally destroyed with its component which added after calling destroy"
    );
    strictEqual(
        childComp.isValid,
        false,
        "node will destroyed with its child components"
    );
    strictEqual(child.isValid, false, "node will destroyed with its children");
    strictEqual(
        newChildNode.isValid,
        false,
        "node will destroyed with its new children"
    );
...
test("attach events", function () {
    ...
    strictEqual(
        child.uuid in attachedNodes,
        false,
        "child node should not attach scene if parent also detached"
    );

    parent.parent = scene;

    strictEqual(
        parent.uuid in attachedNodes,
        true,
        "parent node should attach to scene"
    );
    strictEqual(
        child.uuid in attachedNodes,
        true,
        "child node should attach to scene"
    );
    ...
    strictEqual(
        parent.uuid in attachedNodes,
        false,
        "child node should also detached if parent become detached"
    );
    strictEqual(
        child.uuid in attachedNodes,
        false,
        "parent node should detached"
    );
