(function () {
    ...
    test("unpack from empty package", function () {
        cc.assetManager.packManager.unpack(
            [],
            [1, 0, 0, [], 0, []],
            ".json",
            null,
    ...
    test("pack and unpack something", function () {
        ...
        ok(
            res.indices.indexOf(KEY1) !== -1 &&
                res.indices.indexOf(KEY2) !== -1,
            "should generate index data when packing"
        );
    ...
    test("unpack unused script", function () {
        ...
        var JSON1 = Editor.serializeCompiled(new Class1(), {
            stringify: false,
        });
        var JSON2 = Editor.serializeCompiled(new Class2(), {
            stringify: false,
        });
        ...
        cc.assetManager.packManager.unpack(
            ...
            null,
            function (err, data) {
                var json1 = data[KEY1 + "@import"];
                var json2 = data[KEY2 + "@import"];

                var deserializeCompiled =
                    cc._Test.deserializeCompiled.deserialize;
                var obj1 = deserializeCompiled(json1);
                ok(
                    obj1 instanceof Class1,
                    "Could deserialize json1 when other script not loaded"
                );

                cc.js._setClassId(KEY2, Class2);
                var obj2 = deserializeCompiled(json2);
                ok(
                    obj2 instanceof Class2,
                    "Could deserialize json2 once script loaded lazily"
                );
            }
        );
...
(function () {
    ...
    test("unpack from empty package", function () {
        cc.assetManager.packManager.unpack(
            [],
            { type: cc.js._getClassId(cc.Texture2D), data: "" },
            ".json",
            null,
            function (err, data) {
                var res = data["non-exists key"];
    ...
    var tex1Json = Editor.serializeCompiled(tex1, {
        stringify: false,
        noNativeDep: false,
    });
    ...
    var tex2Json = Editor.serializeCompiled(tex2, {
        stringify: false,
        noNativeDep: false,
    });
    ...
    test("pack and unpack something", function () {
        ...
        ok(
            res.indices.indexOf(tex1._uuid) !== -1 &&
                res.indices.indexOf(tex2._uuid) !== -1,
            "should generate index data when packing"
        );
        ...
        cc.assetManager.packManager.unpack(
            ...
            function (err, data) {
                deepEqual(
                    data[tex1._uuid + "@import"],
                    tex1Json,
                    "should unpack tex1"
                );
                deepEqual(
                    data[tex2._uuid + "@import"],
                    tex2Json,
                    "should unpack tex2"
                );
            }
