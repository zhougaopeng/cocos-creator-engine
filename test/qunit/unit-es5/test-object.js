...
test("cc.isValid", function () {
    ...
    strictEqual(
        cc.isValid(obj2, true),
        false,
        "indicates whether it will be destroyed"
    );
});
...
test("destroy other at destroy callback", 3, function () {
    ...
    obj1._onPreDestroy = function () {
        ...
        strictEqual(
            obj2.isValid,
            true,
            "other is valid until the end of next frame"
        );
    };
    ...
    obj2._onPreDestroy = function () {
        ok(
            true,
            "should called other's destroy callback at the end of next frame"
        );
    };
    ...
    strictEqual(
        obj2.isValid,
        false,
        "other should destroyed at the end of next frame"
    );
});
...
test("destruct", function () {
    ...
    Sub.prototype.test_function = function () {
        return 342;
    };
    ...
    inherited1._destruct();
    ok(
        !inherited1.object_value &&
            !inherited1.string_value &&
            !inherited1.function_value,
        "should remove instance value"
    );
    ...
    ok(
        !newObj.object_value_by_default,
        "should always reset properties event if type changed on-the-fly"
    );
    ok(
        !newObj.dynamicProp,
        "should remove instance value even if defined in ctor"
    );
});
