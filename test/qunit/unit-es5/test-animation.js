...
test("curve types", function () {
    ...
    close(
        entity.x,
        cc.easing.cubicInOut(0.2) * 100,
        0.0001,
        "should wrap time by cc.easing.cubicInOut"
    );

    state.update(1.2);
    close(
        entity.x,
        bezierByTime([0, 0.5, 0.5, 1], 0.4) * 100 + 100,
        0.0001,
        "should wrap time by bezierByTime"
    );
...
test("DynamicAnimCurve", function () {
    ...
    strictEqual(
        target.height,
        10,
        "The keyframe value whose ratio is out of ranges should just clamped"
    );
});
...
test("AnimationState.getWrappedInfo", function () {
    ...
    deepClose(
        ...
        0.00001,
        "should at the end of first loop"
    );
    ...
    deepClose(
        ...
        0.00001,
        "should at the end of second loop"
    );
...
test("initClipData", function () {
    ...
    strictEqual(
        posCurve.target,
        entity,
        "target of posCurve should be transform"
    );
    strictEqual(posCurve.prop, "pos", "propName of posCurve should be pos");
    strictEqual(
        colorCurve.target,
        renderer,
        "target of colorCurve should be sprite renderer"
    );
    strictEqual(
        colorCurve.prop,
        "testColor",
        "propName of colorCurve should be testColor"
    );

    deepEqual(
        posCurve.values,
        [v2(50, 100), v2(100, 75), v2(100, 50)],
        "values of posCurve should equals keyFrames"
    );

    deepEqual(
        colorCurve.values,
        [Color.BLACK, Color.WHITE],
        "values of colorCurve should equals keyFrames"
    );

    deepEqual(
        posCurve.ratios,
        [0, 0.5, 1],
        "ratios of posCurve should equals keyFrames"
    );
    deepEqual(
        colorCurve.ratios,
        [0, 1],
        "ratios of colorCurve should equals keyFrames"
    );
});
...
test("Animation Component", function () {
    ...
    strictEqual(
        entity.x,
        50,
        "target property should equals value in frame 5s"
    );
    ...
    strictEqual(
        !!(animation._animator && animation._animator.isPlaying),
        false,
        "animation should be stopped after load scene"
    );
});
...
test("sampleMotionPaths", function () {
    ...
    strictEqual(
        values[0] instanceof cc.Vec2,
        true,
        "motionPath item should be cc.Vec2"
    );
    ...
    close(
        ...
        0.1327874,
        0.0001,
        "value[index].x should equal value"
    );
    close(
        values[index].y,
        3.8064457,
...
test("SampledAnimCurve", function () {
    ...
    deepEqual(
        entity.position,
        v3(0, 0, 0),
        "entity position should be (0, 0, 0)"
    );
    ...
    deepEqual(
        entity.position,
        v3(100, 100, 0),
        "entity position should be (100, 100, 0)"
    );
...
test("stop Animation", function () {
    ...
    strictEqual(
        animationManager._anims.array.length,
        1,
        "playing animators should be 1"
    );
    strictEqual(
        animation._animator._anims.array.length,
        1,
        "playing anims should be 1"
    );
    ...
    strictEqual(
        animationManager._anims.array.length,
        0,
        "playing animators should be 0"
    );
    strictEqual(
        animation._animator._anims.array.length,
        0,
        "playing anims should be 0"
    );
...
test("play Animation", function () {
    ...
    strictEqual(
        moveState.isPlaying,
        false,
        "move animation state should not be playing"
    );
    strictEqual(
        rotateState.isPlaying,
        false,
        "rotate animation state should not be playing"
    );
    ...
    strictEqual(
        moveState.isPlaying,
        true,
        "move animation state should be playing"
    );
    strictEqual(
        rotateState.isPlaying,
        false,
    ...
    strictEqual(
        rotateState.isPlaying,
        true,
        "rotate animation state should be playing"
    );
...
test("animation enabled/disabled", function () {
    ...
    strictEqual(
        animation._animator.isPlaying,
        true,
        "move animation should be playing"
    );
    strictEqual(
        animation._animator.isPaused,
        true,
        "move animation should be paused"
    );
    ...
    strictEqual(
        animation._animator.isPaused,
        false,
        "move animation should be resumed"
    );
...
test("animation removeClip", function () {
    ...
    strictEqual(
        animation._clips.indexOf(clip1),
        0,
        "clip1 will not be removed since clip1 is defaultClip"
    );
    ...
    strictEqual(
        animation._clips.indexOf(clip1),
        -1,
        "clip1 will be removed even it't defaultClip if force is true"
    );
    strictEqual(
    ...
    strictEqual(
        animation._clips.indexOf(clip2),
        0,
        "clip2 will not be removed since clip2 is playing"
    );
    ...
    strictEqual(
        animation._clips.indexOf(clip2),
        -1,
        "clip2 will be removed even clip2 is playing if force is true"
    );
...
test("animation pause/resume", function () {
    ...
    strictEqual(
        animation._animator._isPaused,
        false,
        "animation should not be paused"
    );
...
test("animation pause/resume should remove animation-actor from animation manager", function () {
    ...
    var state = animation.play("test");
    strictEqual(
        manager._anims.array.length,
        1,
        "should add 1 animation to animation manager"
    );
    ...
    strictEqual(
        manager._anims.array.length,
        0,
        "should remove animation from animation manager"
    );
    strictEqual(
        state.animator,
        null,
        "should unbind animator to animation state when pause animation"
    );
    ...
    strictEqual(
        state.animator,
        animation._animator,
        "should rebind animator to animation state when pause animation"
    );
...
test("animation play on load", function () {
    ...
    strictEqual(
        animation.getAnimationState("clip1").isPlaying,
        false,
        "default clip should not be played if there is playing animation"
    );
    strictEqual(
        animation.getAnimationState("clip2").isPlaying,
        true,
        "should play the specified animation"
    );
...
test("quickFindIndex", function () {
    ...
    var ratio = 0.7;
    strictEqual(
        cc._Test.quickFindIndex(ratios, ratio),
        7,
        "should find ratio at index 7"
    );
});
